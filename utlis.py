...
def valTrackbars(wT=480, hT=240):
    widthTop = cv2.getTrackvarPos("Width Top", "Trackbars")
    heightTop = cv2.getTrackvarPos("Height Top", "Trackbars")
    widthBottom = cv2.getTrackvarPos("Width Bottom", "Trackbars")
    heightBottom = cv2.getTrackvarPos("Height Bottom", "Trackbars")
...
def getHistogram(img, minPer=0.1,display= False,region=1):
    ...
    if display:
        ...
        for x,intensity in enumerate(histValues):
            cv2.line(img,(x,imgHist.shape[0]),(x,imgHist.shape[0] - intensity//255//region),(255,0,255),1)
            cv2.circle(imgHist,(basePoint,img.shape[0]),20,(0,255,255,255),cv2.FILLED)
